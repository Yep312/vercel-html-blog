...
//23.12.16 update: add ctrl tips -- By Yeppioo

function rightMenuYeppioo() {
}
...
rmf.yinyong = function () {
    document.querySelector(".el-textarea__inner").value += `  ${window.getSelection().toString().replace(/(^\s*)|(\s*$)/g, "")}}  `
...
function searchinThisPage() {
    yeppioo.openSearch()
    setTimeout(function () {
        let input = document.querySelector('.local-search-box--input')
...
function popupMenu() {
    ...
    window.oncontextmenu = function (event) {
        ...
        //判断是否是音乐
        // if (metingJs.contains(event.target) || true) {
...
rmf.webSearch = function () {
    ...
    for (const [symbol, encoding] of Object.entries(symbols)) {
        const regex = new RegExp(symbol, 'g');
        encodedStr = encodedStr.replace(regex, encoding);
    }
    window.open("https://cn.bing.com/search?q=" + encodedStr)
    window.location.reload()
}